...
> **The application lives in [`swat/`](./swat/)** — a pnpm + Turborepo monorepo
> (NestJS + Next.js + Postgres/Prisma). Run all package-manager commands from
> there. **New here? Jump to [Quick start](#quick-start).**

## Repository layout

This git repo root is the **outer** `projects/swat/`; the monorepo is the inner `swat/`.

```
projects/swat/                 ← git repo root (you are here)
├── swat/                      ← THE MONOREPO — apps/web, apps/backend, packages/, infra/
...
├── old_swat/                  Legacy CodeIgniter app (reference only; not built)
└── .github/workflows/         CI (runs with working-directory: swat)
```

Because the git root and the monorepo differ, **CI** uses `working-directory: swat`
and **Husky** is enabled with `git config core.hooksPath swat/.husky`.
...
```bash
cd swat                       # everything runs from the inner monorepo
...
> Ports default to backend **:3000** / web **:3001** (set in `swat/.env.local`,
...
deploy): **[`swat/README.md`](./swat/README.md)**. Per-app run/setup:
**[`swat/apps/backend/README.md`](./swat/apps/backend/README.md)** ·
**[`swat/apps/web/README.md`](./swat/apps/web/README.md)**.

## Environment files (.env)

`setup.sh` copies these from their committed `*.example` templates (all are
gitignored). Edit them for your machine; the **repo-root `swat/.env.local` is the
single source of truth** for ports/URLs/secrets in dev.

| File | Copied from | Purpose |
| --- | --- | --- |
| `swat/.env.local` | `swat/.env.example` | Backend runtime + DB/Redis/S3 URLs, secrets, web API base URL |
| `swat/apps/web/.env.local` | `swat/apps/web/.env.example` | Web `NEXT_PUBLIC_API_BASE_URL` (standalone runs; `start.sh` overrides from root) |
| `swat/apps/backend/prisma/.env` | `swat/apps/backend/prisma/.env.example` | `DATABASE_URL` for Prisma CLI (migrate/seed/studio) |
| `swat/infra/docker-compose.env` | `swat/infra/docker-compose.env.example` | Container defaults + host ports (must match `.env.local`) |
| `swat/apps/backend/.env.staging` | `…/.env.staging.example` | Staging DB + legacy MySQL creds for `db:seed:staging` (gitignored) |
| `swat/apps/backend/.env.production` | `…/.env.production.example` | Production cutover creds for `db:seed:production` (gitignored) |
