 ...
 * (0 = Sangat lemah … 4 = Sangat kuat). The hard backend requirement is ≥8
...
export interface PasswordStrength {
  ...
  /** True when the policy (≥8 chars + all character classes) is satisfied. */
...
export function scorePassword(password: string): PasswordStrength {
  ...
  return {
    ...
    meetsRequirements: password.length >= 8 && hasLower && hasUpper && hasDigit && hasSymbol,
  };
}
