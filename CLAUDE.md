...
- **Deployment**: canonical spec [`specs/15-deployment.md`](specs/15-deployment.md); operational
  ...
  **Staging runs 09:00–17:00 WIB only** (EventBridge Scheduler, `infra/aws/provision-schedule.sh`;
  `--disable` to pause) — anything driving the box or DB (deploy, reseed, psql-over-SSM) needs both
  started first outside that window. ~$22.30/mo scheduled vs ~$47.70 always-on; the ~$9.30 floor
