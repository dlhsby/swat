# Phase 5 — Field/Mobile + GPS

## Overview

Enable drivers and field operators to capture trip data offline on mobile devices, with live GPS tracking and push notifications. Build on Phase 1 transaction workflows (trip recording, verification). Implement offline-first PWA with service worker, local queue (IndexedDB), and conflict resolution. Add basic live location ingestion and vehicle position view. Full route-corridor deviation alerting deferred to RFC-0002 (Phase 6 candidate).
...
- Foundation for route-deviation alerting (Phase 6 / RFC-0002).
...
#### T-501. IndexedDB schema & sync queue (client-side)
...
#### T-502. Service worker with background sync (client-side)

- **Size:** M · **Coverage:** ≥75%
- **Depends on:** T-501 (IndexedDB in place), Phase 1 (PWA manifest/sw stub)
- **Files:**
...
- **Steps:**
  ...
  2. **Sync logic:**
     ...
     - On conflict (409, trip already updated): merge strategy (see T-503).
...
#### T-503. Conflict resolution & merge strategy (client-side)

- **Size:** M · **Coverage:** ≥85%
- **Depends on:** T-502 (sync mechanism)
- **Files:**
...
- **Steps:**
  ...
  2. **Implement:**
     ...
     - For Phase 5: `server-wins` for weights (authority), `merge` for notes.
  3. **Test (TDD):**
...
#### T-504. Offline UI state management (client-side)

- **Size:** M · **Coverage:** ≥80%
- **Depends on:** T-501 (IndexedDB), T-503 (conflict resolver)
- **Files:**
...
#### T-505. Trip recording form (responsive, polymorphic)

- **Size:** L · **Coverage:** ≥85%
- **Depends on:** Phase 1 (T-146 Record-trip form backend), T-504 (offline state)
- **Files:**
...
#### T-506. Haul assignment (depart/return) form (mobile)

- **Size:** M · **Coverage:** ≥80%
- **Depends on:** Phase 1 (T-125, T-126 record depart/return endpoints), T-505 (form patterns)
- **Files:**
...
- **Steps:**
  1. **Design:**
     ...
     - Both: require GPS location (optional; see T-509 for integration).
...
#### T-507. Transaction day detail view (mobile-optimized)

- **Size:** L · **Coverage:** ≥80%
- **Depends on:** Phase 1 (T-143 Transaction day list & detail backend), T-505, T-506 (forms)
- **Files:**
...
#### T-508. Offline indicator & sync status (UI)

- **Size:** S · **Coverage:** ≥80%
- **Depends on:** T-504 (offline context)
- **Files:**
...
#### T-509. GPS location capture (client-side, battery-optimized)

- **Size:** M · **Coverage:** ≥80%
- **Depends on:** Phase 1 (Trip recording endpoints), T-504 (offline context)
- **Files:**
...
#### T-510. GPS location ingestion endpoint (backend)

- **Size:** M · **Coverage:** ≥85%
- **Depends on:** Phase 1 (Trip + Haul endpoints), T-509 (GPS capture working)
- **Files:**
...
#### T-511. Vehicle position view (map, basic live tracking)

- **Size:** M · **Coverage:** ≥75%
- **Depends on:** T-510 (GPS readings stored), Phase 2 (dashboards done or partly done)
- **Files:**
...
#### T-512. GPS trail backend service (breadcrumb queries)

- **Size:** M · **Coverage:** ≥85%
- **Depends on:** T-510 (GpsReading table)
- **Files:**
...
#### T-513. Web push service backend integration
...
#### T-514. Push subscription & permission (client-side)

- **Size:** M · **Coverage:** ≥80%
- **Depends on:** Phase 1 (PWA manifest + SW), T-513 (backend service)
- **Files:**
...
#### T-515. Notification center & history (UI)

- **Size:** S · **Coverage:** ≥75%
- **Depends on:** T-513, T-514 (notifications stored + received)
- **Files:**
...
#### T-516. GPS permission request & privacy notice

- **Size:** S · **Coverage:** N/A
- **Depends on:** T-509 (GPS capture), T-514 (push permissions)
- **Files:**
  - `docs/PRIVACY-NOTICE-GPS.md` (create)
  - `apps/web/src/components/GpsPermissionPrompt.tsx` (create)
- **Steps:**
  1. **Privacy notice:**
     - Explain GPS data collection: "Location data is captured to track haul progress and optimize routes (Phase 6). Data is retained for 30 days, then archived."
...
#### T-517. Offline app data security (local storage audit)

- **Size:** S · **Coverage:** N/A
- **Depends on:** T-501 (IndexedDB), T-509 (GPS capture)
- **Files:**
...
- **Steps:**
  ...
  2. **Risk:** device theft → attacker can access IndexedDB (view trips, GPS).
     - Mitigation: app-level encryption optional (Phase 6 if needed); rely on device security.
...
#### T-518. Offline + sync E2E tests (Playwright)

- **Size:** M · **Coverage:** ≥85%
- **Depends on:** T-505, T-506, T-507 (forms + UI complete), T-502 (sync mechanism)
- **Files:**
...
#### T-519. Mobile responsiveness testing

- **Size:** S · **Coverage:** N/A
- **Depends on:** T-505, T-506, T-507 (forms + UI), T-511 (map)
- **Files:**
...
#### T-520. Field/GPS documentation & deployment guide
...
#### T-521. RFC-0002 GPS route-corridor design review & scoping

- **Size:** S · **Coverage:** N/A
- **Depends on:** T-510 (GPS readings stored + queryable), T-512 (GPS trail service)
- **Files:**
  - `docs/RFC-0002-IMPLEMENTATION-NOTES.md` (create — Phase 5 review of RFC)
- **Steps:**
  1. **Review RFC-0002** (in 14-proposals/).
  2. **Phase 5 scope vs Phase 6:**
     - Phase 5 (done): GPS capture + live map + breadcrumb trail.
     - Phase 5 gap: route corridor definition, deviation detection, alerting.
     - Phase 6 scope: route corridor (polyline) per route; geofence logic; alert trigger; supervisor dashboard.
  3. **Note dependencies:**
     - Route table: need corridor (geometry) column + indexes.
     - GpsReading: queryable by haulAssignmentId + time window.
     - Both ready for Phase 6.
  4. **Document:** Phase 5 foundation, Phase 6 roadmap.
- **Acceptance criteria:**
  - [ ] RFC-0002 reviewed in context of Phase 5 completion.
  - [ ] Phase 5 vs Phase 6 scope clarified.
  - [ ] Database ready for Phase 6 (GpsReading, Route corridor).
...
- [ ] **RFC-0002 review:** Phase 5 foundation vs Phase 6 route-deviation alerting scope clarified.
...
**End of Phase 5 — Field/Mobile + GPS Foundation Complete.** Drivers and field operators can:
...
System handles network disruptions gracefully (queue → sync). Mobile UI optimized for field use (large touch targets, single-column on small screens). All GPS data secured locally (browser sandbox); privacy-compliant (notice + data retention policy). Foundation ready for Phase 6: route-corridor deviation alerting (RFC-0002).

---

## Task Summary (T-501 … T-521)

| Task ID | Epic | Title | Size |
|---------|------|-------|------|
| T-501 | 5.1 | IndexedDB schema & sync queue (client-side) | M |
| T-502 | 5.1 | Service worker with background sync (client-side) | M |
| T-503 | 5.1 | Conflict resolution & merge strategy (client-side) | M |
| T-504 | 5.1 | Offline UI state management (client-side) | M |
| T-505 | 5.2 | Trip recording form (responsive, polymorphic) | L |
| T-506 | 5.2 | Haul assignment (depart/return) form (mobile) | M |
| T-507 | 5.2 | Transaction day detail view (mobile-optimized) | L |
| T-508 | 5.2 | Offline indicator & sync status (UI) | S |
| T-509 | 5.3 | GPS location capture (client-side, battery-optimized) | M |
| T-510 | 5.3 | GPS location ingestion endpoint (backend) | M |
| T-511 | 5.3 | Vehicle position view (map, basic live tracking) | M |
| T-512 | 5.3 | GPS trail backend service (breadcrumb queries) | M |
| T-513 | 5.4 | Web push service backend integration | M |
| T-514 | 5.4 | Push subscription & permission (client-side) | M |
| T-515 | 5.4 | Notification center & history (UI) | S |
| T-516 | 5.5 | GPS permission request & privacy notice | S |
| T-517 | 5.5 | Offline app data security (local storage audit) | S |
| T-518 | 5.6 | Offline + sync E2E tests (Playwright) | M |
| T-519 | 5.6 | Mobile responsiveness testing | S |
| T-520 | 5.6 | Field/GPS documentation & deployment guide | S |
| T-521 | 5.7 | RFC-0002 GPS route-corridor design review & scoping | S |

**Total tasks:** 21 | **Est. effort:** 3–4 weeks

---

**Next:** Execute tasks T-501 → T-521 in order, respecting dependencies and parallel groups. Refer to [`08-frontend-spec.md`](../08-frontend-spec.md) for PWA/responsive patterns, [`09-modules/transactions.md`](../09-modules/transactions.md) for trip endpoints, [`06-auth-rbac.md`](../06-auth-rbac.md) for auth patterns, and [`14-proposals/RFC-0002-gps-route-deviation-alerts/`](../14-proposals/RFC-0002-gps-route-deviation-alerts/) for route-deviation scope (Phase 6).
