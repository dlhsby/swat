...
- **CCTV capture upload (legacy `uploadgambar` → `dokumentasitrayek`)**: a weighing carries a `cctvReference` **string** (URL/checkpoint id) on `post-weighing`; to store the actual capture **image** the operator attaches it to the weighing's trip via the generic trip-documentation endpoints — presigned upload (`POST /storage/presigned-put`) then `POST /trips/:id/photos` (`ownerType='trip'`). This is the **per-user bearer** path: the weighbridge desktop app logs in via `POST /auth/token` (not a machine API key) and acts as a real user. Accordingly the **`Petugas Timbang`** role carries `trip:update` (alongside `trip:read`) so it can attach captures — the only trip-mutation right it needs; the weighing itself still posts through `weighbridge:post`. A machine API-key principal (`Integrasi Timbang`) only reaches `/weighbridge/*` and therefore cannot attach image bytes — unattended posting relies on the `cctvReference` string alone.
