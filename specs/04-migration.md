...
**Migration reality (implemented in `scripts/migration/migrate-legacy.ts`):** MD5 is never copied. Every migrated user gets a shared seed password — `LEGACY_SEED_PASSWORD` (default `12345678`) — with `mustChangePassword = true`, so they can sign in (web) to validate their mapped RBAC and are forced to set a real password via `/auth/change-password` (the bearer/native endpoint refuses them until they do). The loader is self-sufficient: `ensureAuthBootstrap()` also seeds the permission catalog + a full-access `admin / 12345678` (also forced to reset, like every other seeded account) plus a `superadmin` (password from the required `SUPERADMIN_PASSWORD` env var — the only role with `system-config:manage`, and the only account that's ready-to-use). Users are **upserted by username** with any collision against a reserved (no-`legacyId`) username suffixed (`admin` → `admin_legacy70`, via `resolveLegacyUsername`). For a real production cutover, override `LEGACY_SEED_PASSWORD` and distribute per-user temporary passwords out-of-band (never in logs). See [`06-auth-rbac.md`](./06-auth-rbac.md) §1.5.1 for the forced-reset flow.
