...
class Tonase extends CI_Controller{
    ...
    public function index(){
        ...
        $this->template->set('title','Monitoring Tonase | SWAT DKP Surabaya');
	...
	public function perdaerah(){
		...
		$this->template->set('title','Jumlah Tonase TPS | SWAT DKP Surabaya');
    ...
    public function getTotalTonaseLimaHari(){
        ...
        $result = $this->model_monitoringtonasesampah->get_all_tonase_sampah_lima_hari($tanggal);
        $hasil = $result->result();
        $dataakhir = array();
	...
	public function getTotalTonaseSatuBulan(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_all_tonase_sampah_satu_bulan($tanggal);
        $hasil = $result->result();
        $dataakhir = array();
        for($i = sizeof($hasil)-1; $i>=0;$i--)
        {
    ...
    public function getTotalTonaseSatuBulanPerKategoriSumberSampah(){
        $tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_all_tonase_sampah_satu_bulan_per_kategori_sumber_sampah($tanggal);
        $hasil = $result->result();
        $dataakhir = array();
        for($i = sizeof($hasil)-1; $i>=0;$i--)
        {
    ...
    public function getNamaTonaseLimaHariTPS(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_tonase_sampah_lima_hari_TPS($tanggal);
    ...
    public function getTonaseLimaHariTPS(){
        ...
        if($result->num_rows() > 0){
            foreach($result->result() as $total){
                $temp = $this->model_monitoringtonasesampah->get_detail_tonase_sampah_lima_hari_TPS($total->NAMA,$tanggal);
                $hasil[] = $temp->result();
            }
            for($i = 4; $i>=0;$i--)
            {
    ...
    /*public function getTonaseLimaHariTPS(){
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_tonase_sampah_lima_hari_TPS();
        $hasil = array();
        $i = 1;
        if($result->num_rows() > 0){
            foreach($result->result() as $total){
                $temp = $this->model_monitoringtonasesampah->get_detail_tonase_sampah_lima_hari_TPS($total->NAMA);
                $hasil[] = $temp->result();
            }
        }
    ...
    public function getSelisihTonase(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_selisih_tonase($tanggal);
        $data = $result->result();
    ...
    public function getTonaseJenisSampah(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampah');
        $result = $this->model_monitoringtonasesampah->get_tonase_jenis_sampah($tanggal);
        echo json_encode($result->result());
    }
}
