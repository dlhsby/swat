...
const STATUS_PILLS: Readonly<Record<PillDomain, DomainMap>> = {
  ...
  // Derived GPS-coverage badge (Phase 7) — keyed by the kebab values the backend
  // returns on the vehicle DTO (`gpsCoverage`).
  gpsCoverage: {
    'tracked-online': { label: 'Terlacak · online', variant: 'green' },
    'tracked-offline': { label: 'Terlacak · offline', variant: 'amber' },
    untracked: { label: 'Tidak terlacak', variant: 'slate' },
  },
  // A single device's freshness (online/offline) on the device registry page.
  gpsDevice: {
    online: { label: 'Online', variant: 'green' },
    offline: { label: 'Offline', variant: 'slate' },
  },
  vehicle: {
