...
- **Entry form (gated by the kind's record permission):** fields mirror legacy per kind. Pool cascades Jenis Aktivitas → Lokasi Pool → **Kendaraan** (the vehicle list only appears once a pool is chosen, filtered to that pool's pending legs). BBM filters the vehicle list to **Dinas** (`D`) vehicles. The **Kendaraan** picker labels each option **`Nopol - Tipe - Model`**. Odometer is **optional** on every kind (no "(opsional)" suffix — the placeholder conveys it; empty ⇒ `-1` sentinel, which skips the odometer-chain check; disposal always sends `-1`). The realization-time field is named per kind — **Waktu Aktivitas** (Pool) / **Waktu Pengambilan** / **Waktu Pembuangan** / **Waktu Pengisian**. Save reuses the scheduled trip (matched by TPS/pool-leg/pending-refuel) or creates an ad-hoc one, then records.
- **Recap grid:** a stable **No.** (ranked by `realizationEntryAt ?? createdAt`, UUID-v7 `id` tiebreak — persists across sort/filter; pinned left) and a **row actions** kebab (Ubah / Hapus, pinned right). **Tipe** and **Model** (from the vehicle's model) show on every kind, right after **Nopol**. Column order per kind: **No · Nopol · Tipe · Model · Pengemudi · …kind columns… · …measures… · [Odometer] · Tanggal · Waktu* · Keterangan · Status (default-hidden) · Aksi** — where _kind columns_ are **Aktivitas · Lokasi Pool** (Pool) / **Bahan Bakar** (BBM) / **TPS** (Pengambilan & Pembuangan); _measures_ are **BBM Diajukan · BBM Disetujui** (BBM) / **Berat Kotor · Berat Kosong · Berat Bersih · CCTV TPA** (Pembuangan); Odometer is omitted on Pembuangan; and **Waktu\*** uses the per-kind label above. **CCTV TPA** renders a "Lihat" trigger that opens the capture (`cctvReference`) in an image modal (legacy screen-icon lightbox parity), falling back to the raw reference if the image can't load. Hidden audit columns (Dibuat/Diubah oleh/pada) match master data.
- **Edit (Ubah):** dialog editing only the realization fields the form captures (measures → odometer → time → notes, mirroring the grid order; per-kind time label), saved via the record endpoint. **Delete (Hapus):** un-record (soft) per `DELETE /trips/:id`.
