function getTotalJenisSampah(tanggal){
	try{
	    ...
	    $.ajax({
	        ...
	        type: "POST",
	        url: base_url+"index.php/monitoring/tonaseswasta/getTonaseJenisSampah",
	        async: false,
	        dataType: 'json',
...
function getTonaseLimaHariTPS(tanggal) {
    ...
    $.ajax({
		data: {tanggal: tanggal},
        type: "POST",
        url: base_url+"index.php/monitoring/tonaseswasta/getTonaseLimaHariTPS",
        dataType: "json",
        async: false,
        success: function (data) {
...
function getNamaTonaseLimaHariTPS(tanggal) {
    try{
	    ...
	    $.ajax({
			data: {tanggal: tanggal},
	        type: "POST",
	        url: base_url+"index.php/monitoring/tonaseswasta/getNamaTonaseLimaHariTPS",
	        dataType: "json",
	        async: false,
	        success: function (data) {
...
function getTonaseLimaHariTotal(tanggal) {
    ...
    $.ajax({
		data: {tanggal: tanggal},
        type: "POST",
        url: base_url+"index.php/monitoring/tonaseswasta/getTotalTonaseLimaHari",
        dataType: "json",
        async: false,
        success: function (data) {
            datatabel = data;
...
function getTonaseSatuBulanTotal(tanggal) {
    ...
    $.ajax({
		data: {tanggal: tanggal},
        type: "POST",
        url: base_url+"index.php/monitoring/tonaseswasta/getTotalTonaseSatuBulan",
        dataType: "json",
        async: false,
        success: function (data) {
            datatabel = data;
		...
		$.ajax({
			...
			success: function (data) {
				$.ajax({
					data: { tanggal: tanggal },
					type: "POST",
					url: base_url + "index.php/monitoring/tonase/getSelisihTonase",
					dataType: "json",
					async: false,
					success: function (datas) {
						...
						alert(data.BERAT);
						alert(datas.BERAT);
						data_berat = (data.BERAT - datas.BERAT);
						alert(data_berat);
						data_selisih = (data.SELISIH - datas.SELISIH);
						data_persen = (data.PERSEN + datas.PERSEN);
						$("#tonaseHariIni").html(data_berat / 1000 + ' TON');
						$("#selisihTonase").html(data_selisih / 1000);
...
function loadSelisihTonase(tanggal) {
	try {
		$.ajax({
			data: { tanggal: tanggal },
			type: "POST",
			url: base_url + "index.php/monitoring/tonaseswasta/getSelisihTonase_total",
			dataType: "json",
			async: false,
			success: function (datas_total) {
					datatabel = datas_total;
					$("#tonaseHariIni").html(datas_total.BERAT / 1000 + ' TON');
					$("#selisihTonase").html(datas_total.SELISIH / 1000);
					if (datas_total.POSISI == "NAIK") $("#persentaseSelisihTonase").html('<i class="fa fa-arrow-up"></i>' + Math.abs(datas_total.PERSEN) + '%');
					else $("#persentaseSelisihTonase").html('<i class="fa fa-arrow-down"></i>' + Math.abs(datas_total.PERSEN) + '%');
					console.log(datatabel);
				}
			});		
	} catch (err) {
