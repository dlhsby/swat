...
| **Pencatatan Aktivitas**        | The four `/record/{pickup,disposal,refuel,pool}` pages collapse into **one tabbed screen** at `/record` (`?tab=` synced, bookmarkable); `QuickEntryBoard` drops its own `PageHead` (parent renders one); nav group → single leaf under **Pengangkutan**.                                                        |
