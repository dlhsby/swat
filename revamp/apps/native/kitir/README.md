...
- A SWAT user account with permissions `disposal-permit:create`, `vehicle:read`, `site:read`.
  (Dev/demo: `administrasi / 12345678` — forced password change on first login, so log into
  the **web app** once first and change the password there; the native token endpoint
  refuses to issue tokens for an account still pending a forced reset.)
