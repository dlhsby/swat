...
JOIN aplikasikendaraan on kategorikendaraan.APLIKASIKENDARAAN_ID = aplikasikendaraan.APLIKASIKENDARAAN_ID
    ...
    function get_all_tonase_sampah_satu_bulan_per_kategori_sumber_sampah($tanggal){
        return $this->db->query("
            SELECT 
                tonase.HARITRANSAKSI_TANGGAL AS TANGGAL, 
                SUM(IF(tonase.SPOT_POOL_ID != '365',tonase.TRAYEK_BERATBERSIHSAMPAH,0)) AS BERATSAMPAH_DINAS,
                SUM(IF(tonase.SPOT_POOL_ID = '365',tonase.TRAYEK_BERATBERSIHSAMPAH,0)) AS BERATSAMPAH_NONDINAS                
            FROM (
            SELECT 
                haritransaksi.HARITRANSAKSI_TANGGAL, 
                kendaraan.KENDARAAN_ID, kendaraan.SPOT_POOL_ID, KENDARAAN_NOMORPOLISI, 
                rute.KATEGORIRUTE_ID, 
                trayek.TRAYEK_BERATKOSONGKENDARAAN, trayek.TRAYEK_BERATKOTORTIMBANGAN, trayek.TRAYEK_BERATBERSIHSAMPAH
            FROM trayek 
            JOIN rute on trayek.RUTE_ID = rute.RUTE_ID 
            JOIN detailtransaksiangkutsampah on trayek.DETAILTRANSAKSIANGKUTSAMPAH_ID = detailtransaksiangkutsampah.DETAILTRANSAKSIANGKUTSAMPAH_ID 
            JOIN transaksiangkutsampah on detailtransaksiangkutsampah.TRANSAKSIANGKUTSAMPAH_ID = transaksiangkutsampah.TRANSAKSIANGKUTSAMPAH_ID 
            JOIN haritransaksi on haritransaksi.HARITRANSAKSI_ID = transaksiangkutsampah.HARITRANSAKSI_ID 
            JOIN kendaraan on transaksiangkutsampah.KENDARAAN_ID = kendaraan.KENDARAAN_ID 
            WHERE rute.KATEGORIRUTE_ID = 4 
            AND MONTH(haritransaksi.haritransaksi_tanggal) = MONTH('".$tanggal."')
            AND YEAR(haritransaksi.haritransaksi_tanggal) = YEAR('".$tanggal."')
            ) tonase
            GROUP BY tonase.HARITRANSAKSI_TANGGAL
            ORDER BY haritransaksi_tanggal DESC
        ");
    }
