...
class Tonaseswasta extends CI_Controller{
    ...
    public function getTotalTonaseLimaHari(){
        ...
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_all_tonase_sampah_lima_hari($tanggal);
        $hasil = $result->result();
        $dataakhir = array();
	...
	public function getTotalTonaseSatuBulan(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_all_tonase_sampah_satu_bulan($tanggal);
        $hasil = $result->result();
        $dataakhir = array();
        for($i = sizeof($hasil)-1; $i>=0;$i--)
        {
    ...
    public function getNamaTonaseLimaHariTPS(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_tonase_sampah_lima_hari_TPS($tanggal);
    ...
    public function getTonaseLimaHariTPS(){
        ...
        if($result->num_rows() > 0){
            foreach($result->result() as $total){
                $temp = $this->model_monitoringtonasesampahswasta->get_detail_tonase_sampah_lima_hari_TPS($total->NAMA,$tanggal);
                $hasil[] = $temp->result();
            }
            for($i = 4; $i>=0;$i--)
            {
    ...
    /*public function getTonaseLimaHariTPS(){
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_tonase_sampah_lima_hari_TPS();
        $hasil = array();
        $i = 1;
        if($result->num_rows() > 0){
            foreach($result->result() as $total){
                $temp = $this->model_monitoringtonasesampahswasta->get_detail_tonase_sampah_lima_hari_TPS($total->NAMA);
                $hasil[] = $temp->result();
            }
        }
    ...
    public function getSelisihTonase(){
        ...
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_selisih_tonase($tanggal);
        $data = $result->result();
     ...
     public function getSelisihTonase_total(){
		$tanggal = $this->input->post('tanggal');
		/*$bulan = $this->input->post('bulan');
        $tahun = $this->input->post('tahun');*/
        $this->load->model('model_monitoringtonasesampahsemua');
        $result_total = $this->model_monitoringtonasesampahsemua->get_selisih_tonase($tanggal);
        $data_total = $result_total->result();
        $berat_total = intval($data_total[0]->BERAT);

        $this->load->model('model_monitoringtonasesampah');
        $result_swasta = $this->model_monitoringtonasesampah->get_selisih_tonase($tanggal);
        $data_swasta = $result_swasta->result();
        $berat_swasta = intval($data_swasta[0]->BERAT);

        $berat_semua = $berat_total-$berat_swasta;
        $berat2 = 1200000;
        $hasil = array();
        if($berat_semua>$berat2){
            $hasil["BERAT"]=$berat_semua;
            $hasil["SELISIH"]=$berat_semua-$berat2;
            $hasil["POSISI"]="NAIK";
            $hasil["PERSEN"]=intval(($hasil["SELISIH"]/$berat2)*100);
        }
        else{
    ...
    public function getTonaseJenisSampah(){
		$tanggal = $this->input->post('tanggal');
        $this->load->model('model_monitoringtonasesampahswasta');
        $result = $this->model_monitoringtonasesampahswasta->get_tonase_jenis_sampah($tanggal);
        echo json_encode($result->result());
    }
}
