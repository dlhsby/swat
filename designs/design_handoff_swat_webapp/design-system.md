...
framework. Light theme is the required baseline; dark mode is optional (Phase 2+).
...
**Reviewable companion:** a living style guide renders every token and component in
`design-system/Design System.html` (open it to see swatches, type specimens, and interactive
component states). The CSS in `design-system/swat-tokens.css` + `swat-components.css` is the
reference implementation of the variable layer below.
...
5. **Added status mapping for `MaintenanceStatus`** (Belum Disetujui / Disetujui).
6. **Added** a formal **z-index scale**, **focus-ring spec**, **shadcn semantic variable layer**,
   and a **number / date / currency conventions** section.
...
`amber` = in-progress / pending · `blue` = done (awaiting verification) · `green` = verified /
good / approved / active · `red` = damaged-critical · `slate` = inactive / lost / archived.
...
Flat by default; elevation only signals floating layers.
...
```
┌──────────────────────────────────────────────────────────────────┐
│ TOPBAR  h64  ·  ☰  logo  "SWAT — DLH Surabaya"        🔔  avatar▾  │ z-sticky
├──────────────┬───────────────────────────────────────────────────┤
│ SIDEBAR w256 │ CONTENT                                            │
│ slate-50     │ ┌ breadcrumb ─────────────────────────────────┐   │
│ groups:      │ │ H1 page title              [primary action] │   │
│ • Dasbor     │ ├──────────────────────────────────────────────┤  │
│ • Master Data│ │ toolbar / cards / table / form               │  │
│ • Penjadwalan│ │ container max-w 1400 · pad 2xl y / xl x       │  │
│ • Transaksi  │ │                                              │   │
│ • Pengguna   │ └──────────────────────────────────────────────┘  │
└──────────────┴───────────────────────────────────────────────────┘
       z-fixed                       canvas: neutral-50
```

**Topbar** — h `64`, bg `white`, border-bottom `1px neutral-200`, `position: sticky; top:0`,
`z-sticky`. Left: hamburger (<lg) + 24px logo + title (`h3`, `neutral-900`). Right: optional
notifications icon, avatar (40px) → dropdown (Profil, Ubah Kata Sandi, Keluar).

**Sidebar** — w `256`, bg `neutral-50`, border-right `1px neutral-200`, `z-fixed`. Nav groups from
`08-frontend-spec` IA (Dasbor / Master Data / Penjadwalan / Transaksi / Pengguna & Akses), filtered
by permission (items the user lacks are **hidden**, not disabled).
- group label: `tiny`, `neutral-400`, uppercase, `tracking-wide`, padding `12px 16px 4px`
- item: `body-sm`, `neutral-600`, padding `8px 16px`, gap `8px`, 20px icon
- hover: bg `neutral-100`, text `neutral-900`
- **active:** bg `primary-50`, text `primary-700`, left border `3px primary-600`, weight 600
- collapses to icon-rail or off-canvas drawer < `lg`; toggle in topbar.

**Content** — canvas `neutral-50`; inner container `max-width 1400`, padding `32px` (2xl) block /
`24px` (xl) inline; sections separated by `24–32px`.

### 2.2 Grid

12-column fluid grid, gutter `24px` (xl), inside the 1400 container. Common spans: forms use a
single column (max `640` readable width) or 2-col field rows on `≥ lg`; dashboard cards 4-up on
`xl`, 2-up on `md`, 1-up on `sm`.
...
Empty / loading(skeleton ×10) / error states per §3.9. Toolbar–table gap `16px`; table–pagination `16px`.

**B. Form** (create/edit)
`breadcrumb → H1 → Card section(s) of FormFields → sticky footer [Batal][Simpan]`.
Fieldsets grouped in cards (pad `16px`, gap `24px` between cards); field→field `16px`; sticky footer
border-top `1px neutral-200`, bg `white`, pad `16px 24px`. Modal variant for simple entities (§3.10).

**C. Detail / Board** (Daily Haul Board)
`H2 + date label + DayStatus pill → one Card row per haul`. Row: plate(mono) · driver · target/actual
times · Ritase · status pill · row actions; verification progress "3/5 Terverifikasi". Row pad `16px`,
gap `12px`. Empty: "Belum ada kendaraan hari ini".

**D. Dashboard**
`H1 greeting → metric Card grid (4-up xl / 2-up md / 1-up sm)`. Card: `label` title, `h1`-size metric
(tabular-nums), optional unit suffix. Decoration kept minimal (Phase 1).
...
- **Sizes:** `sm` 32h / `text-body-sm` / pad `8px 12px` / radius 4 · `md` 40h / `text-body` / pad
  ...
  44px min touch target on the weighbridge kiosk.
...
- Border `1px neutral-200`, radius 6, pad `10px 12px`, `text-body`, bg `white`, h `40`.
- **States:** hover border `neutral-300` · focus border `primary-600` + focus ring · error border
...
- Trigger styled as Input (with chevron, `neutral-400`). Content: bg `white`, radius 6, `shadow-base`,
...
- **Switch** (`<Switch>`): track 36×20 radius-full; off `neutral-300`, on `primary-700`; 16px thumb,
  `200ms` ease. For instant boolean settings (e.g. status Active/Inactive toggles); destructive
  toggles still confirm.
...
- **Empty:** centered icon + "Belum ada data" / "Tidak ada hasil pencarian" + [Buat Baru].
- **Loading:** 10 skeleton rows (shimmer bars at row height).
- **Error:** centered red icon + "Gagal memuat data" + [Coba Lagi] (secondary).

### 3.12 Dialog / Modal — `<Dialog>`
- Backdrop `rgb(0 0 0 / .5)` `z-overlay`; panel `white`, radius 8, `shadow-lg`, `z-modal`,
  ...
  overlay click optional. `role="dialog" aria-modal="true"`, labelled by title.
...
### 3.14 Sheet — `<Sheet>`
- Side panel for heavy/contextual flows (trip detail, multi-field reconciliation). Slides from right
  (desktop) / bottom (mobile), width `420–520`, `shadow-lg`, `z-modal`, `200ms` ease. Same
  header/body/footer contract as Dialog.
...
- Horizontal list; trigger pad `10px 16px`, border-bottom `2px transparent`; active text
  ...
  arrow-key roving focus. Panel pad `16px 0`. Use for verification steps / grouped detail.

### 3.16 Card — `<Card>`
- bg `white`, border `1px neutral-200`, radius 6/8, pad `16px`. Variants: default · clickable
  (hover `shadow-sm`, cursor pointer) · outlined (no shadow) · elevated (`shadow-base`). Optional
  header (title `label`/`h3` + actions), body, footer. Used for form sections, dashboard metrics,
  haul rows.
...
- Bottom-right, `16px` inset, `z-toast`, stack newest-on-top. bg `white`, border-left `4px` (variant
  color), radius 6, `shadow-base`, pad `12px 16px`, `body-sm`, icon + message + close.
- Variants: success ("Berhasil", green) · error ("Gagal", red) · warning ("Peringatan", amber) ·
  info ("Informasi", blue). Auto-dismiss success 3s / error 5s (manual close allowed). `role="status"`
  (success/info) or `role="alert"` (error/warning).
...
- **Purpose:** documentation photos (weighing, vehicle, maintenance — `*Photo` relations).
...
- Track `neutral-200`, radius full, height 8; fill `primary-600`. Determinate (upload %, fuel-quota
  usage) or indeterminate (animated sweep). Pair with a numeric label for screen readers
...
- **Purpose:** read-only key→value detail, especially the **Checker verification summary** (route,
  ...
  mono. This is the canonical read-only counterpart to the Form Field.
...
| focus | primary-600 + ring | white | neutral-900 |
| error | danger-500 + danger ring | white | neutral-900 |
...
:root {
  ...
  /* spacing / type / elevation (non-shadcn, used by app CSS) */
  --font-sans: 'Plus Jakarta Sans','Inter',system-ui,sans-serif;
  --font-mono: 'JetBrains Mono',ui-monospace,monospace;
  --space-lg: 16px; --shadow-base: 0 4px 6px -1px rgb(0 0 0 / .1), 0 2px 4px -2px rgb(0 0 0 / .1);
}
```

> **Dark mode (optional, P2):** add a `.dark { … }` block inverting background/foreground and
> lifting primary to `primary-500/600` for contrast on dark surfaces. Not required for Phase 1.
...
**Touch.** On `pointer: coarse` every interactive control is **≥ 44×44px** (button `md`/`lg`
already satisfy; icon-buttons widen; menu/combobox/bottom-nav rows grow). Keep keyboard parity.

**Safe areas.** Sticky topbar, bottom-nav, and action bars respect
`env(safe-area-inset-top/bottom)` for notch / home-indicator devices.

**Reusable responsive classes** (in `swat-components.css`): `.bottom-nav`, `.drawer`/`.drawer-scrim`,
`.tcards`/`.tcard` (table→cards), `.action-bar`, `.app-banner`. Plus `@media (pointer:coarse)` touch
sizing and `@media (max-width:640px)` dialog→sheet.
...
| `icons` | 192 · 512 · **maskable** (the leaf mark on emerald tile) |
| `orientation` | `any` |
...
- **Dark mode:** follows `prefers-color-scheme` + the user's stored choice.

---

## 7. How to apply

**Theme.** Light theme is the required baseline; build everything light-first. Dark mode is a P2
opt-in via the `.dark` class + localStorage, using the `dark:` variant — do not block Phase 1 on it.
