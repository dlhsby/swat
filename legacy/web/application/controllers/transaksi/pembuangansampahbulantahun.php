...
class Pembuangansampahbulantahun extends CI_Controller{
	...
	public function index(){
		...
		$this->form_validation->set_rules('beratbersih','Berat Bersih Timbangan','trim|required');
		...
		if($this->form_validation->run()==FALSE){
			...
			$orderdate = explode('-', $tanggalHariIni);
			$year = $orderdate[0];
			$month   = $orderdate[1];
			...
			$data['bulan'] = $month;
			$data['monthName'] = $bulans[date('m')];
			$data['tahun'] = $year;
			$this->template->set('title','Pembuangan Sampah TPA | SI Angkutan DKP');
			$this->template->load('template','transaksi/pembuangansampahbulantahun',$data);
		}
		else{
			...
			if($kendaraan->num_rows >0)
			{
				...
				if($kategoriSumberSampahID==1)
					...
					redirect('transaksi/pembuangansampahbulantahun');
			}
	...
	public function getpembuangansampahbyfilter(){
		...
		$all_pembuangansampah = $this->model_trayek->get_all_trayek_pembuangansampah_by_filterbulan($bulan,$tahun,$nopolKendaraan,$aplikasiKendaraan,$kategoriKendaraan,$tps,$statusTrayek);
		$result = $this->model_trayek->get_all_paging_sorting_trayek_pembuangansampah_by_filterbulan($bulan,$tahun,$nopolKendaraan,$aplikasiKendaraan,$kategoriKendaraan,$tps,$statusTrayek,$jtStartIndex,$jtPageSize,$jtSorting);
	...
	public function updatetrayekpembuangansampah(){
		...
		$beratBersih = $this->input->post('TRAYEK_BERATBERSIHSAMPAH');
		...
		if($tpsByNama->num_rows() > 0){
			...
			if ($rute->num_rows() > 0){
				...
				$data_trayek = array(
					...
					'TRAYEK_BERATBERSIHSAMPAH' => $beratBersih,
					'TRAYEK_WAKTUENTRIREALISASI' => $tanggalHariIni.' '.$waktuSekarang,
					'TRAYEK_KETERANGAN' => $keterangan
				);
